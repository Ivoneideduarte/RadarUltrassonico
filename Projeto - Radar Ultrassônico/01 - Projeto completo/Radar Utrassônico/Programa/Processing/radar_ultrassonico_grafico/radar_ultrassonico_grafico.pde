...
import processing.serial.*;       // Importa as bibliotecas para comunicação serial
import java.awt.event.KeyEvent;   // Importa as bibliotecas para ler a informação da entrada da serial
...
PFont orcFont;    // Variável para configurar a fonte do radar

void setup() 
{
 
size (1366, 768); // Aqui é onde você coloca a resolução da sua tela
smooth();

// Aqui tem um detalhe importante:
// onde tem 'Serial.list()[0]', caso dê algum erro
// troque isso pela porta que seu Arduino está, tipo "COM3" ok?

println("Portas seriais disponíveis:");
println(Serial.list());
String COM3 = Serial.list()[0];
Porta_serial = new Serial(this, COM3, 9600);
//Porta_serial = new Serial(this,Serial.list()[0], 9600);   // Começa a comunicação Serial
...
void serialEvent (Serial Porta_serial) 
{
  ...
  //Lê a informação da Serial até ao caracter '.', esta informação é atribuida a uma string chamada "dados".
...
pushMatrix();
  translate(width/2,height-height*0.074); // Move as coordenadas iniciais para outro local
  strokeWeight(9);
  stroke(255,10,10);  // Caso algum objeto seja identificado desenha retângulos vermelhos
  
  pixsDistancia = iDistancia*((height-height*0.1666)*0.025); // Cobre a distância entre o sensor e o objeto
  
  // limite de faixa 40 cm, ou seja, se o objeto estiver a menos de 40 cm, ele aparece na tela.
  if(iDistancia<40){
  
  // Desenha o objeto de acordo com a distância e o ângulo
